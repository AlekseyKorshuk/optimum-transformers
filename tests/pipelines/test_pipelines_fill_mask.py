...
from transformers import MODEL_FOR_MASKED_LM_MAPPING, TF_MODEL_FOR_MASKED_LM_MAPPING
from optimum_transformers import OptimumFillMaskPipeline, pipeline
...
class FillMaskPipelineTests(unittest.TestCase, metaclass=PipelineTestCaseMeta):
    ...
    def get_test_pipeline(self, model, tokenizer, feature_extractor):
        ...
        fill_masker = OptimumFillMaskPipeline(model=model, tokenizer=tokenizer)
        examples = [
    ...
    def run_test_targets(self, model, tokenizer):
        ...
        # Pipeline argument
        fill_masker = OptimumFillMaskPipeline(model=model, tokenizer=tokenizer, targets=targets)
        outputs = fill_masker(f"This is a {tokenizer.mask_token}")
        self.assertEqual(
            outputs,
            [
    ...
    def run_test_top_k(self, model, tokenizer):
        fill_masker = OptimumFillMaskPipeline(model=model, tokenizer=tokenizer, top_k=2)
        outputs = fill_masker(f"This is a {tokenizer.mask_token}")
        self.assertEqual(
            outputs,
            [
